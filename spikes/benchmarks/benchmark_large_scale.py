...
def run_cerebras_benchmarks(graphs, pe_counts, grid_rows, palette_size, compiled_base_dir):
    """
    ...
    for num_pes in pe_counts:
        ...
        compiled_dir = os.path.join(compiled_base_dir, f'csl_compiled_out_p{num_pes}')
...
def main():
    ...
    parser.add_argument('--run-dir', type=str, default=None,
                        help='Run directory root (default: runs/local/benchmark-large-scale)')
    args = parser.parse_args()

    run_dir = args.run_dir or os.path.join(PROJECT_ROOT, 'runs', 'local', 'benchmark-large-scale')
    results_dir = os.path.join(run_dir, 'results')
    compiled_base_dir = os.path.join(results_dir, 'compiled')
    os.makedirs(compiled_base_dir, exist_ok=True)
    ...
    if args.cerebras:
        ...
        cerebras_results = run_cerebras_benchmarks(
            graphs, pe_counts, args.grid_rows, args.palette_size, compiled_base_dir)
    else:
    ...
    with open(os.path.join(results_dir, 'summary.json'), 'w') as f:
        json.dump({
            'mode': 'cerebras' if args.cerebras else 'cpu-simulation',
            'vertex_counts': vertex_counts,
            'pe_counts': pe_counts,
            'avg_degree': args.avg_degree,
            'cpu_results': cpu_results,
            'cerebras_results': {f'{n}:{p}': v for (n, p), v in cerebras_results.items()},
        }, f, indent=2)
