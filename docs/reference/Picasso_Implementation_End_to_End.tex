...
Our first try (\texttt{archive/source-backups/csl/pe\_program.csl.1d.bak}) used the simplest possible approach: pack all boundary wavelets into a send buffer, then fire a \textbf{single bulk DMA transfer} per direction using \texttt{@mov32} with \texttt{.async = true}:

\begin{lstlisting}[language=CSL, caption={Original bulk-send approach (\texttt{archive/source-backups/csl/pe\_program.csl.1d.bak})}, label=lst:oldsend]
...
The second version (\texttt{archive/source-backups/csl/pe\_program.csl.pre-sync.bak}) added 2D grids with north/south support, but \textbf{kept the same bulk DMA approach}. With four directions instead of two, things got worse:
...
The third version (\texttt{csl/variants/sync/pe\_program.csl}) added the hardware row-reduce/broadcast barrier, correctly solving the \emph{synchronization} problem (making sure all PEs finish sending before detect/resolve runs). But it still used bulk DMA for the sends themselves. The barrier got the sequencing right, but could not prevent the FIFO deadlock during the send phase.
