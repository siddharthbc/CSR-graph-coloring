...
RUN_ID="${RUN_ID:-$(date +%Y%m%d)-hardware-${TEST_NAME}-${NUM_PES}pe}"
...
echo "[1/5] Computing per-PE bounds, relay peak, and Picasso T/palette..."
BOUNDS_JSON="$(python3 - "$INPUT_JSON" "$NUM_PES" "$GRID_ROWS" "$HEADROOM" "$PALETTE_FRAC" "$ALPHA" <<'PY'
...
input_json, num_pes, num_rows, headroom, palette_frac, alpha = sys.argv[1:7]
...
print(json.dumps({
    ...
    "derived_T": cur_T, "derived_palette": cur_pal_l0,
}))
PY
)"
echo "    $BOUNDS_JSON"

read MAX_LV MAX_LE MAX_BND MAX_RELAY DERIVED_T DERIVED_PAL <<<"$(python3 -c '
import json,sys
b=json.loads(sys.argv[1])
print(b["max_lv"],b["max_le"],b["max_bnd"],b["max_relay"],b["derived_T"],b["derived_palette"])
' "$BOUNDS_JSON")"
...
# than the wrapper defaults. Hard requirement: artifact max_list_size
# >= largest cur_T any BSP level will request, otherwise the host's
# color_list h2d upload overruns the device symbol and corrupts
# adjacent state -> kernel hangs.
